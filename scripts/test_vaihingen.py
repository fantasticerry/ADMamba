...
def main():
    ...
    if args.tta == "lr":
        ...
        # 多尺度TTA：水平翻转 + 多尺度缩放 (8个变体) - 最适合 ADMamba 模型
        transforms = tta.Compose(
            [
                tta.HorizontalFlip(),
    ...
    OA_no_bg = np.diag(confusion_matrix_no_bg).sum() / (confusion_matrix_no_bg.sum() + evaluator.eps)
    for class_name, class_iou, class_f1 in zip(config.classes, iou_per_class, f1_per_class):
        print('F1_{}:{}, IOU_{}:{}'.format(class_name, class_f1, class_name, class_iou))
    print('F1:{}, mIOU:{}, OA:{}, OA_no_bg:{}'.format(np.nanmean(f1_per_class[:-1]), np.nanmean(iou_per_class[:-1]), OA, OA_no_bg))
    t0 = time.time()
