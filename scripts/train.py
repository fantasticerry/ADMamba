...
# Make repo root importable so `tools.*` and `admamba.*` resolve when running
# this script directly (e.g. `python scripts/train.py -c configs/...`).
...
class Supervision_Train(pl.LightningModule):
    ...
    def forward(self, x, h_map=None):
        ...
        # 对于不支持 h_map 的模型，ADMamba 接口会忽略该参数
        if h_map is not None:
            seg_pre = self.net(x, h_map=h_map)
        else:
            seg_pre = self.net(x)
        return seg_pre
    ...
    def training_step(self, batch, batch_idx):
        ...
        prediction = self.forward(img, h_map=h_map)
        ...
        load_balance_loss = self.net.get_load_balance_loss()
        total_loss = loss + 0 * load_balance_loss  # 0.001是权重
...
def main():
    ...
    trainer = pl.Trainer(devices=config.gpus, max_epochs=config.max_epoch, accelerator='auto',
                         ...
                         logger=logger,
                         accumulate_grad_batches=2)  # Gradient accumulation to maintain effective batch size
