...
def main():
    ...
    # 加载checkpoint，使用strict=False允许部分权重加载
    ...
    print(f"权重加载统计:")
    print(f"  匹配的权重: {len(matched_keys)}/{len(model_state)} ({100*len(matched_keys)/len(model_state):.1f}%)")
    print(f"  缺失的权重: {len(missing_keys)}")
    print(f"  多余的权重: {len(unexpected_keys)}")
    
    if len(missing_keys) > 0:
        print(f"\n警告: 有 {len(missing_keys)} 个模型权重无法从checkpoint加载!")
        print("前10个缺失的权重:")
        for key in missing_keys[:10]:
            print(f"  - {key}")
    
    # 手动加载state_dict，设置strict=False
    missing, unexpected = model.load_state_dict(checkpoint['state_dict'], strict=False)
    if len(missing) > 0:
        print(f"\n实际缺失的权重数量: {len(missing)}")
    if len(unexpected) > 0:
        print(f"实际多余的权重数量: {len(unexpected)}")
