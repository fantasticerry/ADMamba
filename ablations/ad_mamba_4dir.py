...
class CrossScan(torch.autograd.Function):
    ...
    def backward(ctx, ys: torch.Tensor):
        ...
        y_vertical = y_vertical.view(B, -1, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, -1, L)
...
class MambaLayer(nn.Module):
    def __init__(self, in_chs=512, dim=128, d_state=16, d_conv=4, expand=2, last_feat_size=16):
        ...
        #####################################################
        #YTY修改1：增加Mamba2模块，对4向扫描的结果进行处理
        # 四项扫描，每个方向使用独立的Mamba2
        self.mambas = nn.ModuleList([
            ...
            ) for _ in range(4)  # 4个方向：横向、竖向、横向反向、竖向反向
        ])
        #####################################################
    def forward(self, x): # B, C, H, W
        ...
        ys = ys.view(B, 4, chs, H, W)  # (B, 4, C, H, W)
